...
function init(swaggerPath, options) {
  ...
  const schemaBuilderOptions = Object.assign({}, options, {
    buildRequests: true,
...
async function initAsync(swaggerPath, options) {
  ...
  schemas = await apiSchemaBuilder.buildSchema(
    swaggerPath,
...
function _baseInit(swaggerPath, options) {
  ...
  framework = frameworkToLoad
    ? require(`./frameworks/${frameworkToLoad}`)
    : require('./frameworks/express');
...
function _validateRequest(requestOptions) {
  requestOptions.path = requestOptions.path !== '' ? requestOptions.path : '/';
...
function _validateBody(requestOptions) {
  ...
  const methodSchema =
    schemaEndpointResolver.getMethodSchema(schemas, path, method) || {};
...
function _validateParams(requestOptions) {
  ...
  const methodSchema = schemaEndpointResolver.getMethodSchema(
    schemas,
    path,
    method
  );
  if (
    ...
    !methodSchema.parameters.validate({
      query: query,
      headers: headers,
      path: pathParams,
      files: files,
